...
def write_html_report(results, output_path, project_path):
    sections = ["ai_sdks", "llm_models", "local_models", "vector_dbs", "ai_endpoints", "risks"]
    section_titles = {
        "ai_sdks": "AI SDKs",
    ...
    def _format_cell(col, value):
        ...
        if col in {"instances"}:
            return f"<span class=\"count-badge\">{html.escape(str(value))}</span>"
...
def write_excel_report(results, output_path):
    ...
    workbook_parts.append("<Row><Cell><Data ss:Type=\"String\">Section</Data></Cell><Cell><Data ss:Type=\"String\">Count</Data></Cell></Row>")
    for section in sections:
        count = len(results.get(section, []))
        workbook_parts.append(
            "<Row>"
            f"<Cell><Data ss:Type=\"String\">{xml_escape(section)}</Data></Cell>"
    ...
    for section in sections:
        ...
        if not rows:
            workbook_parts.append("<Row><Cell><Data ss:Type=\"String\">No findings</Data></Cell></Row>")
            workbook_parts.append("</Table></Worksheet>")
            continue

        columns = sorted({key for row in rows for key in row.keys()})
        header_cells = "".join(
            f"<Cell><Data ss:Type=\"String\">{xml_escape(col)}</Data></Cell>" for col in columns
        )
        workbook_parts.append(f"<Row>{header_cells}</Row>")

        for row in rows:
            value_cells = "".join(
                f"<Cell><Data ss:Type=\"String\">{xml_escape(row.get(col, ''))}</Data></Cell>" for col in columns
            )
